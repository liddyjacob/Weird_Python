	...
	def make_sets(self, pset, deltalist, tried_sets):
		...
		for (prime, exp) in new_pset:
			if (exp != 1):
				...
				self.try_down(pset, deltalist, index, tried_sets)
				
			

		for (prime, exp) in node.primeset_above():
			#try messing with the exponents
			if (exp != 1):
				self.make_sets(node.primeset_above(), this_deltalist, set_index, tried_sets)
